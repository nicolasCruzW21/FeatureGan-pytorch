...
class CycleGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_A', 'cycle_A', 'D_B', 'G_B', 'cycle_B','F_B', 'F_A', 'G', 'idt_A']
        ...
        if self.isTrain:
            self.model_names = ['G_A', 'G_B', 'D_A', 'D_B', 'D_A_G']
        ...
        if self.isTrain:
            ...
            self.jitter = torchvision.transforms.ColorJitter(brightness=0.1, contrast=0.1, saturation=0.05, hue=0.05)
            ...
            self.optimizer_D = torch.optim.Adam(itertools.chain(self.netD_A.parameters(), self.netD_A_G.parameters(), self.netD_B.parameters()), lr=opt.lr, betas=(opt.beta1, 0.999))
    ...
    def forward(self):
        ...
        if self.isTrain:
            ...
            #self.fake_A_array = self.netG_B(self.add_background_foreground_channel(self.real_B, self.real_B))# G_B(B)
            self.fake_A_array = self.netG_B(self.real_B)# G_B(B)
            #self.fake_A, _ = self.remove_background((self.fake_A_array[0,:]).unsqueeze(0),self.real_B, self.backgroundFactor)
            self.fake_A = (self.fake_A_array[0,:]).unsqueeze(0)
...
#----------------------------------------foreground-----------------------------------------------
    ...
    def backward_G(self,epoch):
        ...
        rec_B_no_Background, normFactor = self.remove_background(self.rec_B, self.real_B, self.backgroundFactor)
        out7_r_B, out14_r_B, out23_r_B, out32_r_B  = self.calculate_Features(rec_B_no_Background)
        out7_f_A, out14_f_A, out23_f_A, out32_f_A  = self.calculate_Features(fake_A_no_Background)
        self.loss_F_B = self.criterionFeature(out7_r_B, out14_r_B, out23_r_B, out32_r_B, out7_f_A, out14_f_A, out23_f_A, out32_f_A) * normFactor

        # GAN loss D_B(G_B(B))
        self.loss_G_B = self.criterionGAN(self.netD_B(self.fake_A), True)
        
        #realBB = util.tensor2im(self.real_B)
        #recBB = util.tensor2im(self.rec_B)
        #util.save_image(realBB, "realB.png")
        #util.save_image(recBB, "recB.png")
    ...
    def optimize_parameters(self,epoch):
        ...
        self.set_requires_grad([self.netD_A, self.netD_A_G, self.netD_B], False)  # Ds require no gradients when optimizing Gs
        self.optimizer_G.zero_grad()  # set G_A and G_B's gradients to zero
        self.backward_G(epoch)             # calculate gradients for G_A and G_B
        self.optimizer_G.step()       # update G_A and G_B's weights
        # D_A and D_B
        self.set_requires_grad([self.netD_A, self.netD_A_G, self.netD_B], True)
