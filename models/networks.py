...
class NLayerDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, alternate=False):
        ...
        if(alternate):
            print("increasing the size of the discriminator")
            sequence += [
    ...
    def forward(self, input):
        ...
        squeezed = self.squeeze_leaky(self.squeeze(input_no_RGB))#Compress the feature maps using 1x1 convolutions
        newInput = torch.cat([input[:,0:3,:], squeezed], 1)#concatenate with the RGB image to form the IT tensor.
